...
# ── Stream 1: Document categories ────────────────────────────────────────────
ALLOWED_CATEGORIES = ("HR", "Finance", "Cloud", "Technical", "Non-Technical")
DEFAULT_CATEGORY = "Uncategorized"  # never store None — frontend expects a string
...
def _normalize_category(raw: Optional[str]) -> str:
    ...
    if raw in ALLOWED_CATEGORIES:
        return raw
    if raw.lower() == DEFAULT_CATEGORY.lower():
        return DEFAULT_CATEGORY
    return DEFAULT_CATEGORY
...
async def upload_document(
    ...
    # Stream 1: Category required at upload (legacy uploads bypass — but the
    # API now mandates it for any new upload).
...
async def list_documents(
    ...
    # Stream 4: ALL filters compose into a single MongoDB AND query.
    # No client-side post-filtering — server returns the exact set.
    and_clauses: List[dict] = []

    # Visibility scope: editors only see own + assigned. Owners/admins see all.
    ...
    # Stream 4: text search composed with all other filters
    if q:
        and_clauses.append({
            "$or": [
    ...
    if category and category != "all":
        # Legacy docs use DEFAULT_CATEGORY virtually — match both missing field
        # and explicit value to keep backward compatibility (Stream 1).
    ...
    if uploaded_by and uploaded_by not in ("all", ""):
        target = user["id"] if uploaded_by == "self" else uploaded_by
        and_clauses.append({"owner_id": target})
    if access and access not in ("all", ""):
        if access == "self":
            and_clauses.append({"owner_id": user["id"]})
        elif access == "assigned":
            and_clauses.append({
                "assigned_to": user["id"],
                "owner_id": {"$ne": user["id"]},
            })

    query: dict = {"$and": and_clauses} if and_clauses else {}

    cursor = documents.find(query, {"_id": 0, "disk_path": 0}).sort("created_at", -1)
    docs = await cursor.to_list(500)
    # Stream 1: backfill DEFAULT_CATEGORY in response for legacy docs
    for d in docs:
        if not d.get("category"):
            d["category"] = DEFAULT_CATEGORY
    return docs
...
# ---------------------------------------------------------------------------
# Stream 2 — Bulk Document Assignment (admin only)
# ---------------------------------------------------------------------------
...
async def bulk_assign_documents(
    ...
    """Assign one or more editors to multiple documents at once.

    Idempotent: re-running the same payload does not create duplicate entries.
    Only admins (ROLE_OWNER) may invoke. Emits one audit log per (doc, editor)
    pair so the trail is fully reconstructable.
    """
    if not body.document_ids:
        raise HTTPException(status_code=400, detail="document_ids cannot be empty")
    if not body.editor_ids:
        raise HTTPException(status_code=400, detail="editor_ids cannot be empty")
    if len(body.document_ids) > 500:
        raise HTTPException(status_code=400, detail="Too many documents (max 500)")

    # Validate editor accounts (only allow real editor/owner users)
    valid_editor_ids: List[str] = []
    cursor = users_coll.find(
        {"id": {"$in": body.editor_ids}, "role": {"$in": [ROLE_EDITOR, ROLE_OWNER, "owner"]}},
        {"_id": 0, "id": 1},
    )
    valid_editor_ids = [u["id"] async for u in cursor]
    if not valid_editor_ids:
        raise HTTPException(status_code=400, detail="No valid editor accounts in request")

    # Idempotent batch upsert: $addToSet with $each prevents duplicates.
    result = await documents.update_many(
        {"id": {"$in": body.document_ids}},
        {"$addToSet": {"assigned_to": {"$each": valid_editor_ids}}},
    )

    await log_event(
        "document.bulk_assign",
        actor_id=actor["id"],
        actor_role=actor["role"],
        resource_type="document",
        ...
        metadata={
            ...
            "editor_count": len(valid_editor_ids),
